...
export default function DestinatariosPage() {
    ...
    const fetchItems = async () => {
        try {
            const res  = await fetch(`${API}/destinatarios`, { headers });
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            const url    = editando ? `${API}/destinatarios/${editando}` : `${API}/destinatarios`;
        ...
        <div className="flex min-h-screen bg-[#020617] text-white overflow-hidden">
            ...
            <div className="flex-1 p-5 md:p-10 overflow-auto relative w-full min-w-0">
                ...
                <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} className="mb-12 flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl md:text-6xl font-black text-cyan-300 drop-shadow-[0_0_25px_rgba(0,255,255,0.5)]">DESTINATARIOS</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
                    {[
                        ...
                        <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                            ...
                            <FaMapMarkerAlt className={`text-4xl ${s.color}`} />
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                    ...
                    <h2 className="text-3xl font-black text-cyan-300 mb-8 flex items-center gap-3"><FaMapMarkerAlt /> Destinatarios Registrados</h2>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody>
                                {filtrados.length === 0 && (
                                    <tr><td colSpan={7} className="py-10 text-center text-gray-500">No hay destinatarios registrados</td></tr>
                                )}
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
                            ...
                            <div className="flex justify-between items-center p-8 pb-0 flex-shrink-0">
                                <h2 className="text-3xl font-black text-cyan-300">{editando ? "Editar Destinatario" : "Nuevo Destinatario"}</h2>
                            ...
                            <div className="flex-1 overflow-y-auto px-8 py-6">
                                <div className="grid grid-cols-2 gap-5">
                                    ...
                                    <Field label="Nombre comercial">
                                        <input value={form.razonSocial} onChange={set("razonSocial")} className={inputCls} />
                                    </Field>
                                    <Field label="RFC">
                                    ...
                                    <Field label="Persona de contacto">
                                        <input value={form.contacto} onChange={set("contacto")} className={inputCls} />
                                    </Field>
                                    ...
                                    <Field label="Requisitos de descarga">
                                        <input value={form.requisitosDescarga} onChange={set("requisitosDescarga")} placeholder="Rampa, montacargas..." className={inputCls} />
                                    </Field>
                            ...
                            <div className="flex gap-4 p-8 pt-4 flex-shrink-0">
                                ...
                                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleSubmit} disabled={loading}
                                    ...
                                    <FaCheck /> {loading ? "Guardando..." : editando ? "Actualizar" : "Guardar Destinatario"}
                                </motion.button>
                            </div>
                        </motion.div>
                    </motion.div>
