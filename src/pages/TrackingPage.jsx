...
const truckIcon = L.divIcon({
    className: "",
    html: `<div style="background:#06b6d4;border:3px solid #fff;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 0 15px rgba(6,182,212,0.8)">馃殯</div>`,
    iconSize: [36, 36], iconAnchor: [18, 18]
});

const origenIcon = L.divIcon({
    className: "",
    html: `<div style="background:#22c55e;border:3px solid #fff;border-radius:50%;width:30px;height:30px;display:flex;align-items:center;justify-content:center;font-size:14px;box-shadow:0 0 10px rgba(34,197,94,0.8)">馃摝</div>`,
    iconSize: [30, 30], iconAnchor: [15, 15]
});

const destinoIcon = L.divIcon({
    className: "",
    html: `<div style="background:#f59e0b;border:3px solid #fff;border-radius:50%;width:30px;height:30px;display:flex;align-items:center;justify-content:center;font-size:14px;box-shadow:0 0 10px rgba(245,158,11,0.8)">馃弫</div>`,
    iconSize: [30, 30], iconAnchor: [15, 15]
});
            ...
            <div className="flex-1 p-10 overflow-auto relative">
                ...
                <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-400 mt-3 text-xl">Monitoreo log铆stico en tiempo real</p>
                    </div>
                ...
                <div className="mb-6 flex gap-4 items-end flex-wrap">
                    ...
                    {viajeSeleccionado && (
                        <div className="flex gap-4 text-sm pb-1">
                            <span className="text-green-400">馃摝 Origen: arrastrable</span>
                            <span className="text-yellow-400">馃弫 Destino: arrastrable</span>
                            <span className="text-cyan-400">銆帮笍 Ruta: l铆nea directa</span>
                        </div>
                    )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-8">
                        ...
                        <MapContainer center={center} zoom={10} style={{ height: "100%", width: "100%" }}>
                            ...
                            {origenPos && (
                                <DraggableMarker
                                    position={origenPos}
                                    icon={origenIcon}
                                    label={`馃摝 Origen: ${viajeSeleccionado?.origen || ""}`}
                                    onDrag={setOrigenPos}
                                />
                            )}
                            {destinoPos && (
                                <DraggableMarker
                                    position={destinoPos}
                                    icon={destinoIcon}
                                    label={`馃弫 Destino: ${viajeSeleccionado?.destino || ""}`}
                    ...
                    <div className="bg-white/5 border border-cyan-500/20 rounded-3xl p-6 overflow-auto h-[600px]">
                        <h2 className="text-2xl font-black text-cyan-300 mb-6 flex items-center gap-2">
                            <FaMapMarkerAlt /> Veh铆culos Activos
                        </h2>
                        ...
                        {viajeSeleccionado && (
                            <div className="mb-4 p-4 rounded-2xl bg-cyan-500/10 border border-cyan-400/20">
                                ...
                                <p className="text-gray-300 text-xs mt-1">{viajeSeleccionado.origen} 鈫?{viajeSeleccionado.destino}</p>
                        ...
                        <div className="flex flex-col gap-4">
                            {locations.length === 0 && (
                                <p className="text-gray-500 text-center mt-10">Sin veh铆culos en tracking</p>
                            )}
                            {locations.map(loc => (
                                <motion.div key={loc.id} whileHover={{ scale: 1.02 }}
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-bold">馃殯 {loc.vehicleName}</h3>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-2xl font-black text-cyan-300">Agregar Veh铆culo GPS</h2>
                                <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-white text-xl"><FaTimes /></button>
                            </div>
                            <div className="flex flex-col gap-4">
                                <div>
                                    <label className="text-gray-400 text-sm mb-2 block">Veh铆culo del cat谩logo</label>
                                ...
                                <div className="p-3 rounded-xl bg-cyan-500/5 border border-cyan-400/10">
                                    <p className="text-cyan-400 text-xs">馃挕 Tip: Abre Google Maps, haz clic derecho en el punto y copia las coordenadas.</p>
                                </div>
                            </div>
